...
public:
	...
	~BpLedService() { delete remote; }

	void LedOn() { int msg, reply; remote->transact(1, &msg, &reply); }
	void LedOff() { int msg, reply; remote->transact(2, &msg, &reply); }
};


// BpLedService의 객체를 생성하는 함수


extern "C" __declspec(dllexport) ILedService* CreateProxy()
{
	return new BpLedService;
}


// dll 빌드하는 법
// cl LedService.cpp /LD /link user32.lib gdi32lib kernel32.lib
// LedService.dll 생성됨
